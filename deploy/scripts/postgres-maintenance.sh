...
prepare_backup_dir() {
    ...
    [[ -d "$backup_host_dir" && -w "$backup_host_dir" ]] || fail "backup directory is not writable: $backup_host_dir"
}

run_backup_unlocked() {
    compose run --rm --no-deps -e BACKUP_LOCK_HELD=1 backup "$@"
}
...
restore_database() {
    ...
    local postgres_db backup_host_dir service
    ...
    [[ -n "$postgres_db" ]] || fail "POSTGRES_DB is not configured"
    [[ "$confirmation" == "prod:${postgres_db}" ]] || fail "confirmation must be prod:${postgres_db}"
